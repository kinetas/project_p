...
CREATE TABLE IF NOT EXISTS financial_statement (
    ...
    fs_div                  VARCHAR(5)   NOT NULL COMMENT '재무제표구분 (CFS/OFS)',
    period_nm               VARCHAR(50)      NULL COMMENT '기간명 (예: 제 50 기)',
    period_dt               VARCHAR(50)      NULL COMMENT '기간일자',
    current_assets          BIGINT           NULL COMMENT '유동자산',
    non_current_assets      BIGINT           NULL COMMENT '비유동자산',
    total_assets            BIGINT           NULL COMMENT '자산총계',
    current_liabilities     BIGINT           NULL COMMENT '유동부채',
    non_current_liabilities BIGINT           NULL COMMENT '비유동부채',
    ...
    operating_income        BIGINT           NULL COMMENT '영업이익',
    income_before_tax       BIGINT           NULL COMMENT '법인세차감전 순이익',
    net_income              BIGINT           NULL COMMENT '당기순이익(손실)',
    currency                VARCHAR(10)      NULL COMMENT '통화단위',
    PRIMARY KEY (id),
    UNIQUE KEY uq_fs_stock_year (stock_code, bsns_year),
    KEY idx_fs_stock (stock_code),
    CONSTRAINT fk_fs_company FOREIGN KEY (stock_code) REFERENCES company (stock_code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='DART 재무제표 피벗 데이터 — (stock_code, bsns_year) 당 1행';
